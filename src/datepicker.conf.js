...
angular.module('app').config(['pikadayConfigProvider', function(pikaday) {
    ...
    pikaday.setConfig({
        ...
        i18n: {
            ...
            /* eslint-disable */
            months        : (ld._months.standalone || ld._months).map(momentLang === 'ka' ? returnItem : capitalize),
            weekdays      : (ld._weekdays.standalone || ld._weekdays).map(momentLang === 'ka' ? returnItem : capitalize),
            weekdaysShort : (ld._weekdaysShort.standalone || ld._weekdaysShort).map(momentLang === 'ka' ? returnItem : capitalize)
            /* eslint-enable */
        },
        firstDay: 1
    });
